...
stripe.api_key = 'sk_test_51M2enSExnmTJCNDxvS2Msp504WvzZBCNwQ4pMnBBti2ceorTtSe2PvkbfiNTaqp0BrueMUAdMH1O5SMqAzCOs05I00Yj1qPYOg'
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        plan = 'price_1M2esrExnmTJCNDxlXDumanq'
        ...
        if request.POST['coupon'] in coupons:
            ...
            try:
                ...
                pass
            subscription = stripe.Subscription.create(customer=stripe_costumer.id, items=[
                                                      {'plan': plan}], coupon=request.POST['coupon'].lower())
        else:
            subscription = stripe.Subscription.create(
                customer=stripe_costumer.id, items=[{'plan': plan}])
        customer = Customer()
        customer.user = request.user
        customer.stripeid = stripe_costumer.id
        customer.membership = True
        ...
        if request.method == 'GET' and 'coupon' in request.GET:
            if request.GET['coupon'].lower() in coupons:
                ...
                coupon_dollar = str(coupon_price)[:-2]+'.' + str(price)[-2:]
